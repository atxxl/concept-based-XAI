...
def evaluate_model(model, device, test_loader, criterion):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            loss = criterion(outputs.logits, labels) # for unbalanced data
...
def __main__():    
    ...
    print(f"RoBERTa_sentiment on {device} w. unbalanced dataset.")
    ...
    df_test  = pd.read_csv("/zhome/81/3/170259/data/twitter-2016test-CE.tsv", 
                           sep='\t', header=None, quoting=3)
    df_train['converted_score'] = df_train[2] + 2 #ranging 0-4
    df_test['converted_score']  = df_test[2] + 2
    ...
    # learning_rates = [1e-4, 3e-4, 5e-4, 1e-5, 3e-5, 5e-5]
    ...
    #         run = wandb.init(project="RoBERTa_sentiment_unbalanced_1", # change the w&b log here
    #                          reinit=True,
    ...
    model.train()
    best_accuracy = 0
    run = wandb.init(project="RoBERTa_sentiment_unbalanced_best", # change the w&b log here
                     reinit=False,
                     config={"epochs": epochs,
    ...
    for epoch in range(epochs):
        ...
        for batch in loop:
            ...
            # Evaluate model performance
            avg_loss = total_loss / len(train_loader)
            accuracy, val_loss = evaluate_model(model, device, test_loader, criterion)
    ...
    name = "retrained_RoBERTa_sentiment_unbalanced_acc_" + str(round(accuracy, 3)) + ".pth"
