...
def __main__():    
    ...
    # Plotting
    _, ax = plt.subplots(figsize=(13, 10), dpi=200)
    sorted_params = sorted(param_combo.items(), key=lambda item: max(item[1]), 
                           reverse=True)
    top_5_params = sorted_params[:5]
    for params, accs in top_5_params:
        ax.plot(range(1, epochs+1), accs, label=params)

    ax.set_xlabel('Epochs', fontsize=20)
    ax.set_ylabel('Accuracy', fontsize=20)
    ax.set_title('Top 5 Parameter Combinations by Accuracy', fontsize=24)
    ax.legend(title="Parameter Combinations", loc='best')
    plt.savefig('BERTweet_balance_acc_comparsion.png', format='png')
    plt.tight_layout()
    plt.show()
